...
- `assets/js/particle-bg.js` builds an animated technical-drawing-style SVG (`#bg-particles`) injected via `layouts/_default/baseof.html`; all element styling lives in an inline `<style>` block generated by `styleEl()` (not `custom.css`)
- The top-right dimension label (`.dim-text-top`) is counted up/down via a `requestAnimationFrame` loop (`animateDimText`), kept in sync with the CSS `dim-text-move` keyframe by matching `DIM_ANIM_MS` to that animation's duration
- Several shapes randomly rotate via `startRandomFlip`/`scheduleNextFlip`; rotation state persists across page loads via `sessionStorage` (`particle-bg-flip-state`)
- SVG rebuilds on window resize (debounced) via `build()`
